...
def train_ppo(config: dict):
    ...
    # Create environment with HardcoreWrapper
    env_id = config['env']['name']
    if config['env']['hardcore']:
        env_id = "BipedalWalkerHardcore-v3"
    
    base_env = gym.make(env_id)
    base_env.reset(seed=config['experiment']['seed'])
    ...
    env = HardcoreWrapper(
        ...
        angle_coef=config['env'].get('angle_coef', 0.02),
        angular_vel_coef=config['env'].get('angular_vel_coef', 0.01)
    )
...
def evaluate(agent, config: dict, num_episodes: int = 10) -> list:
    ...
    """
    # Create evaluation environment with HardcoreWrapper
    env_id = config['env']['name']
    if config['env']['hardcore']:
        env_id = "BipedalWalkerHardcore-v3"
    
    base_eval_env = gym.make(env_id)
    base_eval_env.reset(seed=config['experiment']['seed'] + 999)  # Different seed for eval
    
    eval_env = HardcoreWrapper(
        base_eval_env,
        frame_skip=config['env'].get('frame_skip', 4),
        reward_clip_min=config['env'].get('reward_clip_min', -10.0),
        reward_clip_max=config['env'].get('reward_clip_max', 10.0),
