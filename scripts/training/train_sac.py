#!/usr/bin/env python3
"""Train SAC on BipedalWalker - Support for Easy, Hardcore, and Bridges modes

Usage:
    # Easy mode (no obstacles)
    python training/train_sac.py --config configs/sac_easy_gpu.yaml
    
    # Hardcore mode (stumps, ladders, pitfalls - no bridges)
    python training/train_sac.py --config configs/sac_hardcore_gpu.yaml
    
    # Bridges mode (custom obstacles with bridges)
    python training/train_sac.py --config configs/sac_bridges_gpu.yaml
"""
...
def main():
    parser = argparse.ArgumentParser(description="Train SAC on BipedalWalker")
    ...
    try:
        # Load configuration
        logger.info("=" * 60)
        logger.info("SAC BIPEDAL WALKER TRAINING")
        logger.info("=" * 60)
        ...
        model = SAC(
            ...
            ent_coef=algorithm_config.get('ent_coef', 'auto'),
            target_entropy=algorithm_config.get('target_entropy', 'auto'),
