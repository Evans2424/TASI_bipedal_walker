...
Usage:
    ...
    python visualize_bridge_walker.py --checkpoint experiments/checkpoints/.../sac_model_1000000_steps.zip
...
def make_env(render_mode="human", record_video=False, hardcore=True, wrapper_type="shaped"):
    ...
    def _init():
        ...
        # Apply appropriate wrapper based on checkpoint
        if wrapper_type == "optimized":
            # Bridge-optimized wrapper (soft penalties)
            env = BridgeOptimizedWrapper(
                env,
                frame_skip=4,
        ...
        else:  # "shaped" or default
            # Bridge-shaped wrapper (LIDAR-based)
            env = BridgeShapedWrapper(
                env,
                frame_skip=4,
