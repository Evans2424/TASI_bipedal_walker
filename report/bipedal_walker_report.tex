...
where:
\begin{itemize}
    \item $r_t^{\text{progress}}$ encourages forward position progress of the agent.
    \item $r_t^{\text{posture}}$ penalizes large deviations of the hull angle to keep the torso roughly upright.
    \item $r_t^{\text{energy}}$ penalizes the magnitude of motor torques (energy usage).
    \item $r_t^{\text{fail}}$ applies a large negative penalty when the hull touches the ground (agent has fallen).
\end{itemize}
